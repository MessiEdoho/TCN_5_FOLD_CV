...
# MultiScaleTCNWithAttention (M4) architecture. Loads BOTH the M3
# best-backbone JSON (held fixed during the M4 study) and the M4
# best-attention JSON, builds a MultiScaleTCNWithAttention with the
# merged hyperparameters, and trains one fold at a time on the
# 6-partition mouse manifest from create_5fold_cv_splits.py.
#
...
# (drives early stopping, patience=15, no warm-up). After the training
# loop terminates, restore the best-F1 checkpoint and evaluate ONCE on
# the held-out CV partition with an FP32 forward pass (matches the
# parent eval_utils.py 'Layer 3' NaN protection for attention models).
# Reports the eight segment-level metrics (raw, no post-processing)
# per fold and aggregates across folds as mean +/- std.
#
# Five folds at MAX_EPOCHS=100 with patience=15 should fit comfortably
# inside the 13-day wall time used for the parent training runs.
...
# MultiScaleTCNWithAttention, make_loader, train_one_epoch, set_seed.
...
python train_5fold_cv_multiscaleTCNwithATTN.py \
    --cv-manifest     /home/people/22206468/scratch/INPUT_CV_PROJECT/manifest/data_splits_5fold_cv.json \
    --backbone-params /home/people/22206468/scratch/OUTPUT/MODEL3_OUTPUT/MultiScaleTCNtuning_outputs/best_multiscale_params.json \
    ...
    --output-dir      /home/people/22206468/scratch/OUTPUT_CV_PROJECT/MODEL_4_MTCN_ATTN \
    --folds           0,1,2,3,4
